...
- **UUID v7 Primary Keys** — all 17 tables use time-sortable UUID v7 identifiers generated by a custom PostgreSQL `uuid_generate_v7()` function. IDs are chronologically ordered and globally unique — no sequential integers, no enumeration attacks.
...
│   ├── database/
│   │   ├── schema.sql      # Full DDL — 17 tables (UUID v7 PKs)
...
# Back in the project
npm run migrate   # creates all 17 tables with UUID v7 PKs
```
...
<div align="center">
  ...
  **17 tables • UUID v7 primary keys • Fully normalized relational schema**
</div>
...
| `gst_config` | GST percentage by product category |
| `email_queue` | Queued outgoing emails |
