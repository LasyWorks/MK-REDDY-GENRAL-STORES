...
export default function BillingPage() {
  ...
  const fetchInvoice = async (orderId) => {
    ...
    try {
      ...
      setInvoiceError(error.message || "Invoice not found for this order");
      setInvoice(null);
    } finally {
                  ...
                  <div className="max-w-4xl mx-auto text-sm" id="invoice-content">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-2">
                      <div>
                        ...
                        <p className="text-xs font-semibold leading-tight">{invoice.customer?.name}</p>
                      ...
                      <div className="text-right">
                        ...
                        <p className="text-xs leading-tight">Date : {formatDate(invoice.order_date || invoice.created_at)}</p>
                        <p className="text-xs leading-tight">
                          Time : {new Date(invoice.order_date || invoice.created_at).toLocaleTimeString('en-IN', { hour: '2-digit', minute: '2-digit', hour12: true }).toUpperCase()}
                        </p>
                      </div>
                    </div>
                    ...
                    <table className="w-full mb-2 text-xs border-collapse">
                      ...
                      <tbody>
                        ...
                        {invoice.items?.length < 8 && Array.from({ length: Math.max(0, 8 - invoice.items.length) }).map((_, idx) => (
                        ...
                        <table className="w-full text-xs">
                          <tbody>
                            ...
                            <tr>
                              ...
                              <td className="py-0 text-right">{invoice.is_paid ? formatCurrency(invoice.total_amount) : "Rs 0.00"}</td>
                            </tr>
                            <tr>
                              <td className="py-0">Balance</td>
                              <td className="py-0 text-right">{invoice.is_paid ? "Rs 0.00" : formatCurrency(invoice.total_amount)}</td>
                            </tr>
                            <tr>
                    ...
                    <div className="border-t pt-1">
                      <p className="text-xs font-bold leading-tight">Terms and Conditions</p>
