...
export default function MobileHeader() {
  ...
  const doSearch = async (q) => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/products?search=${encodeURIComponent(q)}&limit=6&is_active=true`,
        { cache: "no-store" },
      );
  ...
  const handleChange = (e) => {
    ...
    setLoading(true);
    setTimerRef(setTimeout(() => doSearch(val), DEBOUNCE_MS));
  };
  ...
  return (
    <header className="md:hidden sticky top-0 z-50 bg-white shadow-sm print:hidden">
      ...
      <div className="px-3 pb-2.5 relative">
        <div className="relative flex items-center bg-[#F7F7F7] rounded-full border border-gray-200 focus-within:border-[#16A34A] focus-within:bg-white transition-all duration-200">
          ...
          <input
            ...
            className="flex-1 bg-transparent pl-9 pr-10 py-2.5 text-sm text-gray-800 placeholder-gray-400 outline-none rounded-full"
            autoComplete="off"
          />
          <div className="absolute right-3 flex items-center gap-1.5">
            ...
            {!query && (
              <button
                className="text-gray-400 active:text-[#16A34A] transition-colors"
