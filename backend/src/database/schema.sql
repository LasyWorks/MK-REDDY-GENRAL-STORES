...
-- USERS
CREATE TABLE users (id UUID PRIMARY KEY DEFAULT uuid_generate_v7(), role_id UUID NOT NULL REFERENCES roles(id) ON DELETE RESTRICT, name VARCHAR(100) NOT NULL, phone VARCHAR(15) NOT NULL UNIQUE, email VARCHAR(100) UNIQUE, password_hash VARCHAR(255), user_type VARCHAR(20) NOT NULL DEFAULT 'retail' CHECK (user_type IN ('retail','wholesale','admin')), address TEXT, google_id VARCHAR(255) UNIQUE, profile_picture TEXT, email_verified BOOLEAN DEFAULT FALSE, is_active BOOLEAN DEFAULT TRUE, is_blocked BOOLEAN DEFAULT FALSE, blocked_reason VARCHAR(255), deleted_at TIMESTAMPTZ DEFAULT NULL, last_login_at TIMESTAMPTZ, created_at TIMESTAMPTZ DEFAULT NOW(), updated_at TIMESTAMPTZ DEFAULT NOW());
