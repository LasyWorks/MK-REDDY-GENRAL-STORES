...
class MergeService {
  ...
  static async createSession({
    ...
    if (recent >= 3) {
      ...
      throw ApiError.tooManyRequests(
        "Too many merge attempts. Please try again later.",
      );
    }
    ...
    logger.info(
      `[MergeService] Session ${sessionId} created — phone ${phone} conflict between ${existingUser.email} and ${newEmail}`,
    );
  ...
  static async sendOTPs({ mergeSessionId, ipAddress = null }) {
    ...
    if (!session) {
      throw ApiError.notFound(
        "Merge session not found or has expired. Please restart registration.",
      );
    }
    if (["completed", "cancelled"].includes(session.status)) {
      throw ApiError.badRequest(
        `Merge session has already been ${session.status}.`,
      );
    }
    ...
    if (!existingUser) {
      throw ApiError.notFound(
        "Existing account not found. Please contact support.",
      );
    }
    ...
    if (!allSent) {
      ...
      logger.error(
        `[MergeService] OTP email failures for session ${mergeSessionId}:`,
        failed,
      );
      throw ApiError.internal(
        "Failed to send verification emails. Please try again.",
      );
    }
    ...
    await MergeAudit.log({
      ...
      detail: {
        to: [
          MergeService.maskEmail(newEmail),
          MergeService.maskEmail(existingEmail),
        ],
      },
      ipAddress,
    });
  ...
  static async verifyOTP({ mergeSessionId, side, otp, ipAddress = null }) {
    if (!["primary", "secondary"].includes(side)) {
      throw ApiError.badRequest(
        'Invalid side.  Must be "primary" or "secondary".',
      );
    }
    ...
    if (side === "primary" && session.primary_otp_verified) {
      return {
        ...
        bothVerified: session.secondary_otp_verified,
      };
    }
    if (side === "secondary" && session.secondary_otp_verified) {
      return {
        side,
        alreadyVerified: true,
        bothVerified: session.primary_otp_verified,
      };
    }
    ...
    if (!otpRecord) {
      throw ApiError.badRequest(
        "Verification code has expired or was not found. Please request new codes.",
      );
    }
    ...
    if (otpRecord.attempts >= MergeOTP.MAX_ATTEMPTS) {
      ...
      throw ApiError.tooManyRequests(
        "Too many incorrect attempts. Merge cancelled. Please restart registration.",
      );
    }
    ...
    const bothVerified = updatedSession
      ? updatedSession.primary_otp_verified &&
        updatedSession.secondary_otp_verified
  ...
  static async executeMerge({ mergeSessionId, ipAddress = null }) {
    ...
    if (!existingUser.is_active)
      throw ApiError.forbidden("Primary account is inactive.");
    if (existingUser.is_blocked)
      throw ApiError.forbidden(
        `Primary account is blocked: ${existingUser.blocked_reason || "Contact support"}`,
      );

    const newUserData =
      typeof session.new_user_data === "string"
    ...
    try {
      ...
      await client.query(
        ...
        [
          ...
          session.phone,
          JSON.stringify({
            preservedFields: ["orders", "cart", "profile", "google_id"],
          }),
          ipAddress,
        ],
      );
      ...
      logger.error(
        `[MergeService] executeMerge transaction failed for session ${mergeSessionId}:`,
        err,
      );
      await MergeAudit.log({
        mergeSessionId,
      ...
      throw ApiError.internal(
        "Merge failed due to a server error. Please contact support.",
      );
    } finally {
      client.release();
    }
    ...
    logger.info(
      `[MergeService] Merge completed — session ${mergeSessionId}; primary user ${freshUser.id}`,
    );
  ...
  static async cancelSession({ mergeSessionId, ipAddress = null }) {
    ...
    const existingUser = await User.findById(session.existing_user_id).catch(
      () => null,
    );
