...
const config = {
  ...
  rateLimit: {
    ...
    maxRequests:
      ...
      (process.env.NODE_ENV === "development" ? 50000 : 5000),
  ...
  limits: {
    ...
    retailCustomerMaxQuantity:
      parseInt(process.env.RETAIL_MAX_QUANTITY, 10) || 10,
    wholesaleCustomerMaxQuantity:
      parseInt(process.env.WHOLESALE_MAX_QUANTITY, 10) || 100,
  },
...
if (
  config.jwt.secret.includes("default") ||
  config.jwt.refreshSecret.includes("default")
) {
  throw new Error(
    "CRITICAL: Default JWT secrets detected. Generate secure secrets immediately",
  );
}

if (!config.cors.origin) {
  throw new Error(
    "CRITICAL: CORS_ORIGIN must be explicitly set (use specific domain, not wildcard)",
  );
}
